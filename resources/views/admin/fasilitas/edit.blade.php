    ...
    <div class="card">
        ...
        <div class="card-body">
            <form action="{{ route('fasilitas.update', $fasilitas->id) }}" method="post" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    <label>Judul</label>
                    <input type="text" name="judul" class="form-control @error('judul') is-invalid @enderror" value="{{ old('judul', $fasilitas->judul) }}">
                    @error('judul')
                ...
                <div class="form-group">
                    ...
                    <textarea name="desc"cols="30" rows="10" class="form-control" id="editor1">{{ $fasilitas->desc }}</textarea>
                    @error('judul')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
